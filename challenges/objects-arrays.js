...
const uni = [];
for (let i = 0; i < graduates.length; i++) {
  graduates[i].university.includes("Uni") ? uni.push(graduates[i].university) : [];
}
console.log(uni);
...
zooAnimals.filter(lesser => lesser.population < 5 ? largerPopulation.push(lesser): []
)
...
*/
const populationTotal = zooAnimals.reduce((sum, curr) => sum + curr.population, 0)
